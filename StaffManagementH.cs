...
namespace SWENWindowForm
{
    public partial class StaffManagementH : Form
    {
        ...
        {
            if (sFirstName.Text.Trim() == string.Empty)
            {
            ...
            if (sLastName.Text.Trim() == string.Empty)
            {
                MessageBox.Show("Double click on the existing rows you wish to update/delete", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            ...
            if (iContactNum.Text.Trim() == string.Empty)
            {
                MessageBox.Show("Double click on the existing rows you wish to update/delete", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            ...
            {
                int tempage;
                bool isNumeric = int.TryParse(iContactNum.Text.Trim(), out tempage);
                if (!isNumeric)
                {
            ...
            if (vAddress.Text.Trim() == string.Empty)
            {
                MessageBox.Show("Double click on the existing rows you wish to update/delete", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        ...
        private void dataGridView1_MouseDoubleClick(object sender, MouseEventArgs e)//double click to update
        {
            ...
            sFirstName.Text = dataGridView1.SelectedRows[0].Cells[1].Value.ToString();
            sLastName.Text = dataGridView1.SelectedRows[0].Cells[2].Value.ToString();
